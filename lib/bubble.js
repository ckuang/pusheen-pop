(function () {
  ...
  Bubble.prototype.move = function (angle) {
    var vecY = (30 * Math.sin(angle * RAD))
    var vecX = Math.sqrt(Math.pow(30, 2) - Math.pow(vecY, 2))
    if (angle >= 90) {
      this.centerX += vecX
      this.centerY -= vecY
    } else if (angle < 90) {
  ...
  Bubble.prototype.getNeighbors = function (otherBubbles) {
    ...
    for (var i = 0; i < otherBubbles.length; i++) {
      if (otherBubbles[i].withinSixty(that)) {
  ...
  Bubble.prototype.newPosition = function (position, otherBubble) {
    switch (position) {
      case 1:
        this.centerX = otherBubble.centerX + 30
        this.centerY = otherBubble.centerY - (30 * Math.sqrt(3))
        break;
      case 2:
        this.centerX = otherBubble.centerX + 60
  ...
  Bubble.prototype.setPosition = function (otherBubble, distance) {
    ...
    var deltaY = (30 * Math.abs(otherBubble.centerY - this.centerY)) / distance
    if (deltaX <= 0) {
      if (deltaY >= 15 && this.centerY < otherBubble.centerY) {
        this.newPosition(1, otherBubble)
      } else if (deltaY >= 15 && this.centerY > otherBubble.centerY) {
  ...
  Bubble.prototype.withinSixty = function (otherBubble) {
    var dangerZone = 60
    var distance = this.distanceFrom(otherBubble)
